...
3. 首版不把接收目录做成双向镜像目录，不传播文件系统中的删除和重命名；历史记录的单条删除可由用户选择同步。
...
| 同步码 Pairing Code | 添加设备时生成的一次性 4 位短码，默认 60 秒失效。 |
...
- 主窗口不再额外创建横跨左右区域的独立标题栏；窗口控制、页面标题和应用内容必须呈现为同一连续界面。macOS 保留系统窗口装饰以获得原生交通灯，Linux 关闭系统装饰并使用自绘控制。
- 左侧栏宽度约 224 px，从窗口顶部连续延伸到底部；顶部先放窗口控制按钮，再依次放产品状态、三个一级菜单和在线/离线设备数量摘要。窗口控制区下方不得出现横向分割线。
- macOS 使用系统原生红、黄、绿交通灯，并通过 `titleBarStyle: Overlay`、隐藏窗口标题和自定义交通灯位置，将系统按钮视觉上嵌入侧栏左上角。红灯关闭、黄灯最小化、绿灯执行 macOS 系统全屏；不得在 WebView 中复制或接管这三个按钮。侧栏 Logo 顶部留白应在避开交通灯的前提下保持紧凑，不形成大块空白。窗口圆角、阴影、hover、按下状态与辅助功能全部交给 macOS 管理。
- Ubuntu/Linux 使用自绘最小化、最大化和关闭按钮，放在右侧页面顶部的可用空间中。普通窗口使用透明承载层包裹圆角内容面板，通过 WebView 绘制克制、均匀且不被窗口边界硬裁切的外阴影，使轮廓与 macOS 保持一致；圆角之外不得出现不透明底色。根页面不得产生横向或纵向滚动范围，只有右侧页面内容区可以按需显示内部滚动条。窗口最大化时必须移除透明留白、圆角、边框和阴影，让内容完整贴合屏幕边缘。
- 右侧内容区从窗口顶部直接开始，只显示当前页面自身的标题、页面操作和内容；不得再显示“粘贴板 / 同步文件 / 设置”等重复的独立窗口 Header。页面标题区与页面背景连续，不使用独立底色；粘贴板历史、同步文件和设置页面的 Header 底部统一使用 1 px 中性灰分割线，明确标题操作区与内容区的层级。
- 标题、侧栏顶部空白和不含交互控件的页面顶部区域可拖动窗口；搜索框、按钮、下拉框等交互区域不得触发窗口拖动。Linux 双击可拖动区域切换最大化状态；macOS 遵循系统标题栏和交通灯行为。
- 粘贴板页默认作为启动首页。
- 同步文件页整个内容区域可接收文件拖放。
- 设置页首屏同时显示同步码和按在线/离线分组的设备列表。
- 默认可见内容区域为 1080 × 720 px，最小可见内容区域为 860 × 560 px；Ubuntu 的原生窗口尺寸需额外包含自绘阴影所需的透明留白。窗口可见内容区域低于 960 px 宽时左侧栏缩至 184 px，但不折叠成图标栏。
- 详细视觉 token、组件状态和无障碍规范以 `apps/desktop/src/styles/` 与组件实现为准。
...
│                      │                                                       │
│  ◉ SyncHalo          │  今天                                                 │
│  ● 同步正常 · 2 在线 │  cargo test --workspace                              │
...
上图的红黄绿按钮仅表示 macOS；Ubuntu/Linux 使用同一圆角与外阴影轮廓，但窗口按钮位于右上角，并在最大化时恢复为无圆角、无阴影的满屏矩形。
...
- 任务状态的图标与文案、右侧“再次同步 / 收藏 / 取消”等图标按钮使用同一条 32 px 高的垂直中心线，不得因状态文字行高产生上下错位。
...
- 同步码为一次性四位码，默认 60 秒失效，使用、超时或离开配对流程后立即失效。
- “生成同步码/刷新”右侧显示“加入”按钮；点击后使用与“清空历史”相同的模态框壳，在弹窗内输入一次性同步码。设置页不再常驻展示加入输入框。
- 我的设备固定按“在线、离线”分组；本机标记“当前设备”，且不能撤销自身。
- “添加设备”“我的设备”“文件接收”“同步与历史”“启动与后台”“当前设备”等区块只显示标题，不在标题下重复展示辅助说明；标题列保持窄栏，把宽度优先留给右侧控件。
...
| FR-DIS-008 | 同步文件页与设置页提供立即刷新入口；触发后重新发布并浏览 mDNS 服务，对新发现的可信离线设备主动恢复连接，不伪造在线状态。 | P0 |
...
| FR-SET-007 | 删除同步和收藏同步独立控制且默认关闭；相关设备均开启后，单条历史的相应操作在在线设备间同步，清空历史仍只影响本机。 | P0 |
...
| 历史记录变更 | 是 | 对应开关启用时立即广播 | 否 | 只影响同事件 ID 的历史元数据，不删除接收文件。 |
...
四位同步码由已有成员在本地使用安全随机数生成，只能使用一次，默认 60 秒失效。同步码不能作为普通字符串直接发送给对方，也不能直接派生长期密钥。
...
应用启动时，持久化但没有对应运行任务的 `QUEUED / TRANSFERRING / VERIFYING` 状态必须恢复为可重试的 `FAILED`，不得永久显示为活跃任务。取消是发送端与接收端共同认可的终态；取消后允许保留已完成校验的临时块，但迟到的进度、失败或完成事件不得把任务改回活跃或完成状态。
...
MVP 按顺序写入文件，因此把已校验块持久化为从 0 开始的连续区间；该区间等价于只包含一个连续范围的块 bitmap。重新连接后：

1. 发送端重新发送 `FileOffer`。
2. 接收端返回连续已校验区间的结束偏移及该前缀的 BLAKE3。
3. 发送端重新计算源文件相同前缀的 BLAKE3；匹配时只发送剩余块，不匹配时双方回退到偏移 0。
4. 最终重新执行整文件校验。

后续支持乱序或多流分块时再扩展为压缩 bitmap 或多个缺失区间。接收端只在完整块通过校验后写入临时文件，并按时间或数据量刷新数据，例如每 1 秒或每 64 MiB 一次。
...
应用数据同步不依赖互联网；生产包启动约 5 秒后检查 GitHub Release 更新清单，之后每 30 分钟轮询一次，并在设置中提供手动检查入口。关闭自动更新时，发现新版后显示版本、发布说明与“立即更新 / 忽略此版本”，仅在确认后安装；忽略的版本只抑制启动和定时提醒，用户手动检查时仍需再次显示。macOS 与 Windows 开启自动更新时后台下载并验签，完成后显示“安装并重启 / 稍后”。Ubuntu 仅发布 DEB，确认安装后通过 Polkit 请求管理员授权，受限 helper 刷新固定签名 APT 源、安装精确版本并重启应用；WebView 和普通应用进程不得获得 root 权限。任何平台都不得未经确认重启，失败不能影响本地同步服务，同一进程只允许一个检查、下载或安装任务。
...
22. 设置页首屏同时展示一次性同步码和按在线、离线分组的设备列表；“加入”使用统一模态框，页面无行内加入 UI、自动同步粘贴板开关、通知开关和平台能力行；“同步与历史”提供默认关闭的删除同步、收藏同步开关，页脚只显示应用版本。
...
26. 所有生产包启动后及每 30 分钟检查更新并支持手动检查；关闭自动更新时先显示发布说明并等待“立即更新 / 忽略此版本”，忽略只抑制自动提醒，手动检查仍显示该版本。macOS 与 Windows 开启时自动下载验签后等待“安装并重启”；Ubuntu 在确认后弹出管理员授权，通过受限 helper 和签名 APT 源安装精确版本。任何模式都不得未经用户确认重启。
