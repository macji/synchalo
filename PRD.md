...
发送文件时某台设备离线，任务显示“等待设备上线”。设备重新上线后，在源文件仍存在且未变化的前提下自动续传。剪贴板旧事件不会在设备重新上线后覆盖当前系统剪贴板。
...
│                                                                              │
│  今天                                                                        │
│    cargo test --workspace                 [悬停：复制 收藏 删除]           │
...
- 复制、收藏、删除三个图标默认隐藏，鼠标悬停或键盘焦点进入该行时显示。
- 仅已收藏历史在行最左侧常驻显示实心星星；未收藏不显示状态文字或图标。该星标属于内容状态，不属于悬停操作。
- 顶部“收藏”是可切换筛选项：启用时只显示收藏内容，再次点击恢复全部内容。
- 粘贴板页顶部不重复放置暂停/恢复按钮；全局暂停保留在左侧栏，暂停后的页面提示仍提供“恢复”。
- 复制只能通过悬停操作中的复制图标触发，并显示“已复制”提示。
...
├──────────────────────────────────────────────────────────────────────────────┤
│  ┌───────────────── 我的设备 ────────────────┐ ┌──────── 文件同步 ───────┐ │
│  │ Jason 的 MacBook Air  本机 [显示同步码] │ │ 将同步到 3 台设备        │ │
│  │ ☑ Studio Ubuntu       在线              │ │ 把文件拖入或者直接粘贴文件│ │
│  │ ☑ Desk Pi             在线              │ │ [+ 选择文件]              │ │
│  │ ☑ Office Ubuntu       离线 · 上线后发送 │ │ 复制文件后按 ⌘V 自动同步 │ │
│  └──────────────────────────────────────────┘ └────────────────────────────┘ │
│                                                                              │
│  同步历史                                                                  │
│  SyncHalo-design.zip     传输中 67%                    [再次同步] [☆] [取消] │
│  notes.pdf               已完成                   [再次同步] [★] [打开] […] │
...
- 选择、拖入、粘贴和再次同步在没有远端目标时统一提示：“当前没有可同步的设备，至少需要 2 台设备，才会开启同步”。
...
- 文件历史显示文件名、大小、方向、设备、时间和状态；每行右侧常驻“再次同步”和收藏图标。
...
- 强调色只用于主操作和进行中状态；在线、等待、失败同时使用图标、文字和颜色表达。
...
4. 用户确认发送，或使用默认“发送给所有空间成员”。
5. 在线设备立即开始接收；离线设备进入等待状态。
...
| FR-FILE-003 | 默认发送给全部空间成员，允许取消部分目标。 | P0 |
| FR-FILE-004 | 接收端自动保存，不需要再次确认。 | P0 |
| FR-FILE-005 | 支持传输进度、瞬时速度、平均速度和预计剩余时间。 | P0 |
| FR-FILE-006 | 支持失败重试、网络恢复后续传和 App 重启后恢复任务。 | P0 |
...
| FR-FILE-013 | 没有远端同步目标时，所有文件入口显示统一的“至少需要 2 台设备”提示。 | P0 |
...
- 用户点击历史项复制时会创建一个新的本机剪贴板事件，因此可以再次同步给其他在线设备。
...
| 文件事件 | 是 | 立即 Offer | 是 | 目标上线后继续投递，直到完成、取消或源文件失效。 |
| 文件数据块 | 接收进度落盘 | 流式发送 | 是 | 只写临时文件，全部校验后提交。 |
...
剪贴板事件仅在创建时已经在线的成员间短暂重试，实时窗口默认 5 秒；超过窗口后不再自动投递正文，防止旧内容覆盖用户当前剪贴板。文件和成员事件采用持久重试。
...
17. 粘贴板历史行点击不复制；顶部无重复暂停按钮；已收藏行最左侧常驻显示实心星星，未收藏不显示状态，悬停或键盘聚焦时显示复制、收藏、删除按钮，删除后可在 5 秒内撤销。
...
| 源文件在离线等待期被移动 | 任务无法继续 | 保存指纹并明确失败；P1 引入有配额的 CAS Outbox。 |
...
8. 文件同步支持排队和断点续传，但 MVP 在源文件变化后允许任务明确失败。
