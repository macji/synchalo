...
应用数据同步不依赖互联网；生产包启动约 5 秒后检查 GitHub Release 的签名更新清单，之后每 30 分钟轮询一次，并在设置中提供手动检查入口。用户可关闭自动轮询，手动检查仍可用；发现新版本后自动下载、验签、安装并重启，失败不能影响本地同步服务。同一进程只允许一个更新检查或安装任务。Linux 自动安装仅支持 AppImage，DEB 通过 APT 或手动安装更新。
...
26. 生产包可关闭每 30 分钟一次的自动更新轮询，并可随时手动检查；启用时只安装通过嵌入公钥验证的更新包，并在安装完成后自动重启。三平台更新清单与安装包必须来自同一版本标签提交。
