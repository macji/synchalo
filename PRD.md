...
- Ubuntu/Linux 使用自绘最小化、最大化和关闭按钮，放在右侧页面顶部的可用空间中。普通窗口使用透明承载层包裹圆角内容面板，通过 WebView 绘制克制的外阴影，使轮廓与 macOS 保持一致；圆角之外不得出现不透明底色。窗口最大化时必须移除透明留白、圆角、边框和阴影，让内容完整贴合屏幕边缘。
