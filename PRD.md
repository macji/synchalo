...
- macOS 使用系统原生红、黄、绿交通灯，并通过 `titleBarStyle: Overlay`、隐藏窗口标题和自定义交通灯位置，将系统按钮视觉上嵌入侧栏左上角。红灯关闭、黄灯最小化、绿灯执行 macOS 系统全屏；不得在 WebView 中复制或接管这三个按钮。窗口圆角、阴影、hover、按下状态与辅助功能全部交给 macOS 管理。
...
- 右侧内容区从窗口顶部直接开始，只显示当前页面自身的标题、页面操作和内容；不得再显示“粘贴板 / 同步文件 / 设置”等重复的独立窗口 Header。页面标题区与页面背景连续，不使用独立底色或贯穿式底部分割线。
