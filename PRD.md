...
- 主窗口关闭系统原生装饰，但不再额外创建横跨左右区域的独立标题栏；窗口控制、页面标题和应用内容必须呈现为同一连续界面。
- 左侧栏宽度约 224 px，从窗口顶部连续延伸到底部；顶部先放窗口控制按钮，再依次放产品状态、三个一级菜单和在线/离线设备数量摘要。窗口控制区下方不得出现横向分割线。
- macOS 使用自绘红、黄、绿窗口按钮，并将按钮直接嵌入侧栏左上角；普通窗口使用约 18 px 圆角和系统阴影，最大化时取消圆角。窗口四角之外必须透明，不得用与背景相近的矩形伪装圆角。透明窗口仅在 `tauri.macos.conf.json` 中启用 `macOSPrivateApi`，适用于当前 Developer ID 签名、公证和站外分发路径，不声明支持 Mac App Store。
...
- 标题、侧栏顶部空白和不含交互控件的页面顶部区域可拖动窗口；搜索框、按钮、下拉框等交互区域不得触发窗口拖动。双击可拖动区域切换最大化状态。
- 粘贴板页默认作为启动首页。
- 同步文件页整个内容区域可接收文件拖放。
