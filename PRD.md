...
应用数据同步不依赖互联网；macOS 与 Windows 生产包启动约 5 秒后检查 GitHub Release 的签名更新清单，之后每 30 分钟轮询一次，并在设置中提供手动检查入口。关闭自动更新时，发现新版后显示版本、发布说明与“立即更新 / 忽略此版本”，仅在确认后下载、验签、安装并重启；忽略的版本持久化保存，在更高版本发布前，启动、定时和手动检查均不再弹窗。开启时后台下载并验签，完成后显示“安装并重启 / 稍后”，不得未经确认重启。失败不能影响本地同步服务，同一进程只允许一个检查、下载或安装任务。Ubuntu 仅发布 DEB，通过包内自动配置的签名 APT 源或手动安装更新。
...
26. macOS 与 Windows 生产包每 30 分钟检查更新并支持手动检查；关闭自动更新时先显示发布说明并等待“立即更新 / 忽略此版本”，同一被忽略版本不再提醒；开启时自动下载验签后等待“安装并重启”。任何模式都不得未经用户确认重启，只安装通过嵌入公钥验证且与版本标签提交一致的更新包。Ubuntu DEB 使用同一标签构建并通过签名 APT 源更新。
