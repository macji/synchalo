...
应用数据同步不依赖互联网；生产包默认检查 GitHub Release 的签名更新清单。用户可在设置中关闭自动更新；发现新版本后自动下载、验签、安装并重启，失败不能影响本地同步服务。Linux 自动安装仅支持 AppImage，DEB 通过 APT 或手动安装更新。
...
26. 生产包可关闭自动更新；启用时只安装通过嵌入公钥验证的更新包，并在安装完成后自动重启。三平台更新清单与安装包必须来自同一版本标签提交。
