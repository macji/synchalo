...
When the current host is macOS, every completed change to executable code or build/runtime configuration must include a native macOS build after the relevant frontend and/or Rust tests pass. Before building, inspect and stop any running SyncHalo process from this repository, then run `npm run tauri -- build` with the installed `Developer ID Application` identity. Submit the fresh app through the `SyncHaloNotary` keychain profile, staple the accepted ticket, and verify it with `codesign`, `stapler`, and `spctl`. Stage the fresh bundle, replace `release/macos-arm64/SyncHalo.app` as a whole, rebuild and verify its ZIP, regenerate `SHA256SUMS`, and confirm the canonical app's Finder modification time reflects the current build so the user can open it for manual verification. The task is not complete until all of these checks succeed. Documentation-only changes do not require this build. Do not publish the artifacts unless the task explicitly includes release publication.
