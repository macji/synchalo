...
1. 文件数据改为固定大小的校验块；每块携带偏移、长度和 BLAKE3，接收端验证通过后才写入临时文件，全部接收后再次执行整文件 BLAKE3 校验与原子提交。
2. 断点续传不再只信任临时文件长度：接收端返回连续已校验前缀的长度与哈希，发送端核对源文件前缀；不一致时从 0 开始，匹配时只发送剩余块。
3. 取消操作同时覆盖发送和接收任务，向对端传播取消请求，保留已经校验的临时块供后续续传，并保证迟到的进度、失败或完成事件不能覆盖 `cancelled` 终态。
4. 发送失败或取消后可原任务重试；接收任务可向原发送设备请求重试。应用重启时遗留的活跃任务统一转为可重试失败状态，不在界面中永久停留为传输中。
5. 增加损坏临时前缀、逐块校验、真实续传、双向取消、接收端请求重试和重启恢复测试；完成前端、Rust、跨平台打包与 macOS 签名公证验证后再发布 `v0.1.10`。
6. 收紧 macOS 侧栏 Logo 与窗口顶部的留白；粘贴板历史、同步文件和设置页 Header 增加统一灰色分割线，并保持传输状态与操作图标垂直居中。

## v0.1.9 发布计划

1. macOS 使用系统原生交通灯与 Overlay 标题栏，绿色按钮执行系统全屏；窗口保持原生圆角和阴影，页面标题与侧栏组成一体化界面。
2. Ubuntu/Linux 使用无系统装饰的自绘右上角窗口控制；浏览器回归覆盖无重复 Header、无分割线和平台控制差异。
3. README 保持英文默认入口，提供与英文版完整对应的中文翻译和可复制的 AI 安装提示词；删除过时的独立 UI 设计文档。
4. 完成前端、Rust、浏览器烟测、macOS Developer ID 签名与 Apple 公证，并发布 macOS ARM64、Ubuntu ARM64/x86_64 和 Windows x64 的 `v0.1.9` 正式包。

## v0.1.8 发布计划

1. 新增英语、简体中文、繁体中文、日文与韩文界面资源；新安装默认跟随系统语言，不受支持的系统语言回退为英语。
2. 设置页新增语言选择并持久化 `system / en / zh-cn / zh-tw / ja / ko` 偏好，切换后即时更新界面、日期、状态、错误信息、系统托盘、原生文件选择器和系统通知。
3. 兼容 0.1.7 及更早版本的设置数据，补充语言解析、持久化迁移与五语界面回归测试；README 全面改为英文并采用 MIT License。
4. 完成前端与 Rust 全量门禁、浏览器烟测、macOS Developer ID 签名和 Apple 公证，再发布 macOS ARM64、Ubuntu ARM64/x86_64 与 Windows x64 的 `v0.1.8` 正式包。

## v0.1.7 发布计划

1. Ubuntu ARM64 与 x86_64 DEB 均执行启动约 5 秒、每 30 分钟和手动更新检查，发现新版后显示版本与发布说明。
2. 用户确认安装后通过 Polkit 弹出管理员授权，受限 helper 只允许 APT 从固定签名源安装精确的 `sync-halo` 版本，成功后重启应用。
3. 忽略版本只抑制自动提醒；手动检查仍显示同一版本。GitHub 并行构建两种 Ubuntu 架构，签名 APT 源同时发布 `arm64` 与 `amd64` 索引，再完成 `v0.1.7` 发布。
...
- 在冻结 Protocol v3 前决定把当前长度前缀 JSON 控制帧迁移到 Protobuf，或正式修订 PRD 的编码决策。
